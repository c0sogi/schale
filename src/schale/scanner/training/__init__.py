...
def generate_class_mapping(output_path: Path | None = None) -> ClassMapping:
    ...
    Args:
        output_path: Optional path to save the mapping JSON file.
            Defaults to the shared cache's training/equipment directory.
    ...
    if output_path is None:
        output_path = cache_directory() / "training/equipment/class_mapping.json"
...
def load_class_mapping(path: Path | None = None) -> ClassMapping:
    ...
    Args:
        path: Optional path to the mapping JSON file.
            Defaults to the local training output, or installed inventory resources.
    ...
    """
    if path is None:
        path = cache_directory() / "training/equipment/class_mapping.json"
        if not path.exists():
            path = resolve_resources("inventory") / "models/class_mapping.json"
