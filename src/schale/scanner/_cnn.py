...
Provides a trained MobileNetV3-Small model for classifying equipment icons
...
class CNNClassifier:
    ...
    Uses a trained MobileNetV3-Small model to classify equipment icons from
    ...
    def __init__(
        ...
        Args:
            model_path: Path to the ONNX model file. Defaults to
                the installed inventory resource bundle.
            mapping_path: Path to the class mapping JSON file. Defaults to
                the mapping beside the selected model.
        ...
        if model_path is None:
            model_path = (
                resolve_resources("inventory") / "models/equipment_classifier.onnx"
            )
        if mapping_path is None:
            mapping_path = model_path.with_name("class_mapping.json")
