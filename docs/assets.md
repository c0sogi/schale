...
학습 모델을 넣지 않습니다. 비전 기능은 사용자가 준비한 자원을 사용합니다.
현재 프로젝트가 공개 배포하는 게임 참조 번들은 없습니다. `schale[scanner]`
설치만으로 인벤토리를 인식하거나, 빈 캐시에서 학생 화면을 인식하지는 못합니다.
데이터·계정·보상 도구는 이 자원이 필요하지 않습니다.
