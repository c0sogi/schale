...
def model_paths():
    try:
        models = resolve_resources("inventory") / "models"
    except ValueError:
        pytest.skip("Optional local inventory resources not installed")
    return models / "equipment_classifier.onnx", models / "class_mapping.json"


@pytest.fixture
def prepared_atlas(monkeypatch, tmp_path):
    from schale.scanner import _icons

    icon = tmp_path / "fixture.png"
    import cv2

    assert cv2.imwrite(str(icon), np.zeros((32, 32, 3), np.uint8))
    equipment = SimpleNamespace(Category="Hat", Icon="fixture", Tier=1)
    monkeypatch.setattr(
        _icons, "cache_collection", SimpleNamespace(equipments={1: equipment})
    )
    monkeypatch.setattr(_icons, "_download_icon", lambda *args, **kwargs: icon)
    atlas = _icons.IconAtlas()
    atlas.prepare()
    return atlas


def test_model_files_exist(model_paths):
    """Verify model and mapping files exist."""
    assert all(path.is_file() for path in model_paths)


def test_model_size_constraint(model_paths):
    """Verify ONNX model is < 50MB if it exists."""
    model_path, _ = model_paths
    size_mb = sum(path.stat().st_size for path in model_path.parent.glob("*.onnx*")) / (
        1024 * 1024
    )
...
def test_atlas_preparation_does_not_load_neural_models(prepared_atlas):
    """Reference data preparation stays separate from neural model loading."""
    ...
    assert atlas._templates
    assert all(template.path.is_file() for template in atlas._templates.values())


def test_inference_speed_benchmark(model_paths):
    """Benchmark CNN inference speed if model exists."""
    model_path, mapping_path = model_paths
...
def test_class_mapping_integrity(model_paths):
    """Verify class mapping has 191 classes and all expected categories."""
    with model_paths[1].open() as f:
        mapping = json.load(f)
...
def test_atlas_preparation(prepared_atlas):
...
def test_scan_inventory_integration():
    ...
    for item in result.items:
        ...
        assert (item.quantity is not None) == (item.quantity_status == "exact")
        assert 0 <= item.tier <= 10
...
def test_tier_detection_range():
    """Tier zero represents growth materials; wearable tiers extend through 10."""
    test_image = Path(r"examples\test_grid_001.png")
    if not test_image.exists():
        pytest.skip("Test image not available")
    ...
    for item in result.items:
        assert 0 <= item.tier <= 10, f"Invalid tier {item.tier} for {item.equipment_id}"


def test_quantity_detection_range():
    """Only exact readings carry integers; abbreviations never invent precision."""
    test_image = Path(r"examples\test_grid_001.png")
    if not test_image.exists():
        pytest.skip("Test image not available")
    ...
    for item in result.items:
        if item.quantity_status == "exact":
            assert isinstance(item.quantity, int) and item.quantity >= 0
        else:
            assert item.quantity is None
        if item.quantity_status == "abbreviated":
            assert item.displayed_quantity is not None
...
def test_atlas_reuse_and_resource_switch(monkeypatch, tmp_path):
    """Repeated scans reuse the atlas; selecting other resources invalidates it."""
    ...
    monkeypatch.setattr(module, "_atlas", None)
    monkeypatch.setattr(module, "_atlas_root", None)
    monkeypatch.setattr(module, "_matcher", None)
    monkeypatch.setattr(module, "IconAtlas", Atlas)
    monkeypatch.setattr(
        module, "resolve_resources", lambda component: tmp_path / "first"
    )
    atlas = module._get_atlas()
    assert module._get_atlas() is atlas
    monkeypatch.setattr(
        module, "resolve_resources", lambda component: tmp_path / "second"
    )
    assert module._get_atlas() is not atlas
